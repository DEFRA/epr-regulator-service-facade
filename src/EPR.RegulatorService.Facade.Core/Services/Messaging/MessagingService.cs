...
public class MessagingService : IMessagingService
{
    ...
    private string? SendEmail(string recipient, string templateId, Dictionary<string, object> parameters)
    {
        ...
        catch (Exception ex)
        {
            ...
            string logData = string.Format("GOV UK NOTIFY ERROR. Class: {0}, Method: {1}, Organisation: {2}, Template: {3}", this.GetType().Name, callingMethodName, organisationNumber, templateId);
            _logger.LogError(ex, logData.Replace('\n', '_'), LogLevel.Error);
        }
